...
app.get("/random",(req,res)=>{
  ...
  res.send(JSON.stringify(jokes[random])).status(200);
})
...
app.get("/filter",(req,res)=>{
  ...
  let jokesFiltered=jokes.filter((joke)=>joke.jokeType===type);

  res.send(JSON.stringify(jokesFiltered)).status(200); 
  }catch(err){
    res.status(404).send("Joke not found");
  }
...
app.post("/jokes",(req,res)=>{
  try{
   ...
   const joke={
    id:jokes.length+1,
    jokeText:req.body.text || "Hello",
    jokeType:req.body.type || "default"
  }
    jokes.push(joke)
    res.json(joke);
  }
...
const type=req.body.type;
    ...
    if(!joke){
      res.status(404).send("No jokes to show")
    }
...
app.delete("/jokes/:id",(req,res)=>{
  try {
    ...
    const joke=jokes.find((joke)=>joke.id===id);
    if(!joke){
      res.status(401).send("Joke couldnt be found")
    }else{
      jokes.splice(id,1);
      res.status(201);
    }
...
app.delete("/all",(req,res)=>{
  try{
    ...
    res.status(404).send("Key mismatch")
  }
})
